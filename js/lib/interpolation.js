...
var ResampleLanczos = (function() {
	...
	return function(src, width, height, filterSize, kernel) {
		...
		var windowX = Math.min(Math.max(Math.ceil(filterSize * Math.sqrt(Math.max(1, isx))), filterSize), MAX_WINDOW);
		var windowY = Math.min(Math.max(Math.ceil(filterSize * Math.sqrt(Math.max(1, isy))), filterSize), MAX_WINDOW);
		if (!cache) CACHE = cache = createCache(kernel, cachePrecision, filterSize);
		y = height;

		while (y--) {
			sourcePixelY = (y + 0.5) * isy;
			y1b = sourcePixelY - windowY;
			if (y1b < 0) y1b = 0;
			y1e = y1et = sourcePixelY + windowY;
			...
			while (x--) {
				...
				x1b = sourcePixelX - windowX;
				if (x1b < 0) x1b = 0;
				x1e = x1et = sourcePixelX + windowX;
